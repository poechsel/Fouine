...
let rec readExpr lexbuf env inter_params =
  ...
  | Eol -> Eol, env
  | _ ->  let _ = if inter_params.disp_pretty then begin print_endline @@ beautyfullprint r;print_string "type ";print_endline @@ print_type @@ snd @@ analyse r (Env.create)  end else ()
    in let env'  = begin
        try
