...
let get_id_from_tuple t =
  let rec aux t =
    match t with
    | Tuple (l, _) -> 
      List.fold_left (fun a b ->  a @ aux b)  [] l 
    | FixedType(t, _, _) -> 
      aux t
    | Ident (t, _) -> 
      [t]
...
let rec get_all_ids expr =
  ...
  | Ident (t, _) -> 
    [t]
  | Tuple (l, _) -> 
    List.fold_left (fun a b -> a @ get_all_ids b) [] l
  | Constructor(_, Some expr, _) -> 
    get_all_ids expr
  | FixedType (t, _, _) -> 
    get_all_ids t
  | _ -> []
...
let rec unify ident expr env error_infos = 
  ...
  | FixedType (t, _, _), t' -> 
    unify t t' env error_infos
  | Const a, FInt b when a = b -> 
    env
  | Underscore, _ -> 
    env
  | Unit, FUnit -> 
    env
  | Bool a, FBool b when a = b -> 
    env
  | Ident (ident, _), _ -> 
    Env.add env ident expr
  | Constructor(name, None, er), FConstructor(name', None) ->
    if snd name = snd name' then
      env
    else
      raise (send_error 
               (Printf.sprintf "Can't unify constructors %s with %s" 
  ...
  | Tuple (l1, error), FTuple l2 ->
    if tuple_has_double_id ident then
      ...
      let rec aux l1 l2 env =  begin match  (l1, l2) with
        ...
        | _ -> raise (send_error 
                        (Printf.sprintf "Can't unify %s with %s" 
                           (print_value expr) 
                           (pretty_print_aux ident "" true)) 
                        error_infos )
...
let interpret program env k kE = 
  ...
  let rec aux env k kE program =
    ...
    | FixedType (x, _, _) -> 
      let k' x' _ = k x' env in aux env k' kE x
    ...
    | Constructor(name, None, er) -> 
      k (FConstructor (name, None)) env 
    ...
    | Not (x, error_infos) -> 
      let k' x' _ =
        begin 
          ...
          | _ -> raise (send_error 
                          "Not operations can only be made on boolean values" 
                          error_infos)
        end
      in aux env k' kE x
    ...
    | LetRec(a, b, error_infos) -> 
      begin match a with
        ...
        | _ -> raise (send_error 
                        "a function declaration must begin by an id" 
                        error_infos)
      end
    ...
    | IfThenElse(cond, a, b, error_infos) ->
      let k' cond' _ = 
        begin 
          ...
          | _ -> raise (send_error 
                          "In a If clause the condition must return a boolean"
                          error_infos)
        end
    ...
    | Printin(expr, error_infos) -> 
      let k' a _ = 
        begin
          ...
          | FInt x ->  k (FInt(x)) env
          | _ -> raise (send_error 
                          "This function is called 'prInt'. How could it work on non-integer values"
                          error_infos)
        end 
      in aux env k' kE expr 
    ...
    | ArrayMake (expr, error_infos) ->
      let k' a _ = 
        begin
          ...
          | FInt x when x < 0 -> 
            raise (send_error 
                     (Printf.sprintf "The size (%d) of this array will be negative" x)
                     error_infos)
          | FInt x ->
          ...
          begin match (id', expr') with
            | FArray (x), FInt (i) -> 
              if i < 0 || i >= Array.length x then
                ...
                k (FInt x.(i)) env
            | a, b -> raise (send_error 
                               "Bad way to access an array" 
                               error_infos)
          end 
        in aux env k' kE expr
      in aux env k'' kE id
            ...
            begin match (id', expr', nvalue') with
              | FArray (x), FInt (i), FInt(y) -> (* pensez à ajouter la generation d'exceptions aprés coup *)
                if i < 0 || i >= Array.length x then
                  ...
                  x.(i) <- y; k FUnit env
              | _ -> raise (send_error 
                              "When seting the element of an array, the left side must be an array, the indices an integer and the value an integer" 
                              error_infos)
            end 
          in aux env k' kE expr
        in aux env k'' kE id
    ...
    | MatchWith (expr, match_list, error ) ->
      let k' expr' _ = 
        let rec aux_match l =
          ...
          | [] -> raise (send_error 
                           (Printf.sprintf "Didn't match the expr : %s" 
                              (print_value expr')) 
                           error)
  ...
  in let e, _ = aux env k kE program
  in e, !env_t
