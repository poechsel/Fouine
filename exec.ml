...
(* problem with env : the one of pierre uses keys, the one for secd r machine sometimes looks more like a stack. so for let and r endlet i don't know what to do yet *)
(* dans ma stack il y a :
*  - du code (instr list)
*  - des closures (e * code)
*  - des constantes *)
(* i'm now using dump to store old r env during LET / ENDLET operations *)
...
(* we have an argument stack and a return stack *)
let rec exec s (e, le) code d r r nbi =
  match code with 
  ...
  | instr::c ->
    ...
    match instr with
    | TAILAPPLY -> let CLOS (x, c, e) = pop s in
                    
    | C k -> (push (CST k) s ; exec s (e, le) c d r r (nbi + 1))

    | REF k -> (push (SREF k) s; exec s (e, le) c d r (nbi + 1)) 

    | BANG x ->
        let EnvREF k = Env.get_most_recent e x in begin push (CST !k) s; exec s (e, le) c d r (nbi + 1) end
    ...
    | CLOSURE (x, c') -> ( push (CLOS (x, c', e)) s ; exec s (e, le) c d r (nbi + 1))
    ...
    | UNITCLOSURE (c') -> (push (UNITCLOS (c', e)) s; exec s (e, le) c d r (nbi + 1)) 
    ...
    | APPLY ->
        ...
        | CST v ->  let clos = pop s in 
                    ...
                    begin 
                      ...
                      exec s (e'', x) c' d r  (nbi + 1) (* c' should end by a
                      return which will resume the exec *)
                    end
                    | UNITCLOS (c', e') -> begin
                                            push (ENV (e, le)) s;
                                            push (CODE c) s;
                                            exec s (e', "") c' d r (nbi + 1)
                                           end
                    end
        | UNITCLOS (c', e') ->
            begin
      ...
      (* just put e instead r of e' in the add r e x, we'll see though *)
    ...
    | PRINTIN -> 
        ...
        begin
          match v with
          | CST k -> begin print_int k ; print_string "\n" ; push v s ; exec s (e, le) c d r  (nbi + 1) end
    ...
    | LET x -> 
        ...
        let e' = Env.add e x (env_of_stack v) in
          push (e, le) d; 
          exec s (e', x) c d r  (nbi + 1)
        end
   
    | ENDLET -> let (e', le') = pop d in exec s (e', le') c d r  (nbi + 1)

    | PROG prog_code -> begin push (CODE prog_code) s ; exec s (e, le) c d r (nbi + 1) end

    | BRANCH -> 
        let CODE b = pop s
        in let CODE a = pop s
        in let CST k = pop s
        in if k = 0 then exec s (e, le) (b @ c) d r (nbi + 1)
           else exec s (e, le) (a @ c) d r (nbi + 1)
       
    | TRYWITH  ->
        let CODE b = pop s
        in let CODE a = pop s
        in begin 
        try exec s (e, le) (a @ c) d r (nbi + 1)
        with EXIT_INSTRUCTION -> exec s (e, le) (b @ c) d r (nbi + 1)
        end
